<template>
  <div class="svg-map" @mousemove="handleMouseMove" ref="svg_map">
    <!-- <svg width="1140" height="421"> -->
    <svg width="1140" height="440">
      <!-- <g transform="scale(0.9552847094227923) translate(146.6807532640282, 0)"> -->
      <g transform="scale(1) translate(146.6807532640282, 0)">
        <path
    ...
    <div
      class="country-tips"
      v-show="state.country.name"
      :style="{ left: `${mapX}px`, top: `${mapY}px` }"
    >
      <div class="country-icon" v-if="state.country.icon">
        <img :src="state.country.icon" alt="" />
      </div>
      <div class="country-name">{{ state.country.name }}</div>
      <div class="country-support">
        <img
          v-if="state.country.support"
          src="../../assets/support.png"
          alt=""
        />
        <img v-else src="../../assets/no-support.png" alt="" />
        <span :class="{ 'coming-soon': !state.country.support }">
          {{
            state.country.support
              ? $t('COUNTRY.support')
              : $t('COUNTRY.coming_soon')
          }}
        </span>
      </div>
...
import { SUPPORT_COUNTRY_MAP, COMING_COUNTRY_MAP } from '../../utils/country';

let lastSupportedCountry = null;
let lastComingSoonCountry = null;
const zhNodeList = [];
// 地图的背景颜色 ACTIVE 表示鼠标移上去显示的颜色
const NOT_SUPPORT_COUNTRY_BG = '#E9EAF1';
const NOT_SUPPORT_COUNTRY_BG_DARK = '#595B68';
const SUPPORT_COUNTRY_BG = '#2980FE';
const SUPPORT_COUNTRY_ACTIVE_BG = '#77B0FF';
const COMING_SOON_COUNTRY_BG = '#AAE5F8';
const COMING_SOON_COUNTRY_BG_DARK = '#5DADC6';
const COMING_SOON_COUNTRY_ACTIVE_BG = '#AAE5F8';
const COMING_SOON_COUNTRY_ACTIVE_BG_DARK = '#5DADC6';
...
const notSupportedFill = computed(() => {
  if (isDarkMode.value) {
    return NOT_SUPPORT_COUNTRY_BG_DARK
  } else {
    return NOT_SUPPORT_COUNTRY_BG
  }
})
const supportedFill = computed(() => {
  if (isDarkMode.value) {
    return SUPPORT_COUNTRY_BG
  } else {
    return SUPPORT_COUNTRY_BG
  }
})
const supportedActiveFill = computed(() => {
  if (isDarkMode.value) {
    return SUPPORT_COUNTRY_ACTIVE_BG
  } else {
    return SUPPORT_COUNTRY_ACTIVE_BG
  }
})
const comingSoonFill = computed(() => {
  if (isDarkMode.value) {
    return COMING_SOON_COUNTRY_BG_DARK
  } else {
    return COMING_SOON_COUNTRY_BG
  }
})
const comingSoonActiveFill = computed(() => {
  if (isDarkMode.value) {
    return COMING_SOON_COUNTRY_ACTIVE_BG_DARK
  } else {
    return COMING_SOON_COUNTRY_ACTIVE_BG
  }
})

onMounted(() => {
  nextTick(() => {
    // console.log('svg_map', svg_map.value.offsetTop);
    // const gNode = document.querySelector('g')
    const nodeList = document.querySelectorAll('path');
    if (nodeList && nodeList.length) {
      const obj = {};
      nodeList.forEach((item) => {
        obj[item.dataset.code] = item.dataset.code;
        if (item.dataset.code === 'TW' || item.dataset.code === 'CN') {
          // console.log('中国台湾', item);
          zhNodeList.push(item)
          // item.attributes.fill.value = '#2980FE'
        }
      });
...
const handleMouseMove = (event) => {
  ...
  if (code) {
    ...
    mapX.value = x.value;
    mapY.value = y.value - svg_map.value.offsetTop - 80;
    if (SUPPORT_COUNTRY_MAP[code]) {
      state.country.name = t(`COUNTRY.${code}`);
      state.country.icon = SUPPORT_COUNTRY_MAP[code].icon;
      state.country.support = true;
      if (lastSupportedCountry) {
        // lastSupportedCountry.attributes.fill.value = SUPPORT_COUNTRY_BG;
        lastSupportedCountry.attributes.fill.value = supportedFill.value;
      }
      // 设置支持的国家的区域鼠标移上去的颜色
      // event.target.attributes.fill.value = SUPPORT_COUNTRY_ACTIVE_BG;
      event.target.attributes.fill.value = supportedActiveFill.value;
      lastSupportedCountry = event.target;
    } else {
      if (lastSupportedCountry) {
        // lastSupportedCountry.attributes.fill.value = SUPPORT_COUNTRY_BG;
        lastSupportedCountry.attributes.fill.value = supportedFill.value;
    ...
    if (COMING_COUNTRY_MAP[code]) {
      if (lastComingSoonCountry) {
        // lastComingSoonCountry.attributes.fill.value = COMING_SOON_COUNTRY_BG;
        lastComingSoonCountry.attributes.fill.value = comingSoonFill.value;
      }
      // 设置即将支持的国家的区域鼠标移上去的颜色
      // event.target.attributes.fill.value = COMING_SOON_COUNTRY_ACTIVE_BG;
      event.target.attributes.fill.value = comingSoonActiveFill.value;
      lastComingSoonCountry = event.target;
      if (code === 'TW' || code === 'CN') {
        // zhNodeList.forEach((item) => item.attributes.fill.value = COMING_SOON_COUNTRY_ACTIVE_BG);
        zhNodeList.forEach((item) => item.attributes.fill.value = comingSoonActiveFill.value);
      } else {
        // zhNodeList.forEach((item) => item.attributes.fill.value = COMING_SOON_COUNTRY_BG);
        zhNodeList.forEach((item) => item.attributes.fill.value = comingSoonFill.value);
      }
      if (COMING_COUNTRY_MAP[code].continent) {
        state.country.name = t(`COUNTRY.${COMING_COUNTRY_MAP[code].continent}`);
      }
      if (COMING_COUNTRY_MAP[code].icon) {
        state.country.icon = COMING_COUNTRY_MAP[code].icon;
      }
      state.country.support = false;
    } else {
      if (lastComingSoonCountry) {
        // lastComingSoonCountry.attributes.fill.value = COMING_SOON_COUNTRY_BG;
        lastComingSoonCountry.attributes.fill.value = comingSoonFill.value;
...
.svg-map {
  ...
  .country-tips {
    ...
    background: #ffffff;
    border-radius: 15px;
    box-shadow: 0px 14.71px 22.06px 0px rgba(48, 159, 255, 0.25);
    display: flex;
